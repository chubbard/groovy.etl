...
import gratum.ng.source.CsvSource
import org.junit.Ignore
import org.junit.Test

import static gratum.ng.source.CollectionSource.from
import static gratum.ng.source.CsvSource.csv
import static gratum.source.HttpSource.http
...
"""]
    ...
    void testFilterMapWithCollection() {
        ...
        assert statistic.getRejections( RejectionCategory.IGNORE_ROW, "filter Pclass in [3,2], Sex = male" ) == 209
    }
    ...
    void testBranchWithGroupBy() {
        csv("src/test/resources/titanic.csv")
            .branch { Pipeline p ->
                return p.groupBy("Sex", "Pclass").addStep("test group by") { Map row ->
    ...
    void testRejections() {
        ...
        LoadStatistic stats = from(hobbies)
                .addStep("Rejection") { Map row -> row.id > 1 ? row : Pipeline.reject(row, "${row.id} is too small", RejectionCategory.REJECTION) }
                .onRejection { Pipeline pipeline ->
